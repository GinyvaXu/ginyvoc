...
| 🔗 房间系统 | **单服务器单房间**：取消房间号，主机自选端口，好友首页直接填 IP:端口 加入 |
| 🌐 跨网络 | 蒲公英 / 米西 / ZeroTier 组网，或 SakuraFrp 内网穿透（见文档） |
| 🖧 连接方式 | 主机自选端口（默认 3000）；好友首页输入 IP:端口 直连，重启后自动进入 |
...
1. 输入昵称 → **进入房间**（单房间制，无需房间号；桌面版可在首页选「开房间」端口或填好友的 IP:端口）
...
> 两种思路任选：**组网（虚拟局域网，画面最稳）** 或 **内网穿透（只有开服者要装）**。开服者在首页选「开房间」并填端口（默认 3000），好友在首页选「加入房间」填开服者的 IP:端口 即可。
> 完整图文教程（SakuraFrp 穿透 / 蒲公英·米西·ZeroTier 组网）：[docs/fri-network.md](docs/fri-network.md)

### 方案一：组网（推荐）

任选一个虚拟组网工具，**所有人**装好并加入同一网络；开服者在 GinyScreen 首页选「开房间」填端口（默认 3000），把「本机可访问地址」里的**虚拟 IP:端口** 发给好友；好友在首页选「加入房间」填入 → 自动重启并进入。
...
国内运营商宽带大多已分配公网 IPv6。双方都有 IPv6 时，开服者把 `[IPv6地址]:端口` 发给好友（端口为首页自选值，默认 3000），好友填入「加入房间」即可直连（服务器已默认双栈监听）。
...
| 版本 | 说明 |
|------|------|
| v1.3.0 | 取消房间号：单服务器单房间；首页直接填 IP:端口 加入，主机自选端口（默认3000）；保存配置后自动重启进入 |
