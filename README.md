...
> 两种思路任选：**组网（虚拟局域网，语音最稳）** 或 **内网穿透（只有开服者要装）**。都通过「帮助 → 服务器设置」填写/展示服务器地址。
> 完整图文教程（SakuraFrp 穿透 / 蒲公英·米西·ZeroTier 组网）：[docs/fri-network.md](docs/fri-network.md)

### 方案一：组网（推荐）

任选一个虚拟组网工具，**所有人**装好并加入同一网络；开服者在 GinyVoC「帮助 → 服务器设置」勾选「本机开服」，把「本机可访问地址」里的**虚拟 IP** 发给好友；好友勾选「连接朋友的服务器」填入 → 保存并重启。

| 工具 | 免费额度 | 说明 |
|------|---------|------|
| 蒲公英（贝锐） | 3 台设备 | 国内服务稳定，免费限速 1~2 Mbps，语音（Opus）够用 |
| 米西开黑 | 免费 | P2P 低延迟组网，自带语音/屏幕（也可只用来组网跑 GinyVoC） |
...
> ⚠️ 内网穿透只解决「找到服务器」；语音是 WebRTC P2P 直连**不走隧道**。家庭宽带多数能打洞成功；
> 若双方网络严格（校园网/公司网）可能连不上语音——遇到这种情况改用方案一组网即可。
> GinyVoC 会在打洞失败时弹出诊断提示。
...
组网方案首次运行请允许 Windows 防火墙弹窗；SakuraFrp 方案无需放行入站。

### 有公网 IP / VPS 时（进阶）

1. 服务器部署到公网 VPS（开放 3000 端口）
2. 严格 NAT 下配置 TURN 兜底：
...
| 版本 | 说明 |
|------|------|
| v0.6.0 | 跨网络联机：服务器设置（本机开服/连朋友）、双栈监听、SakuraFrp 穿透 + 蒲公英/米西/ZeroTier 组网、打洞失败诊断 |
