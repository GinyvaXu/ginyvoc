...
![形态](https://img.shields.io/badge/形态-Web%20%2B%20Electron-blue) ![平台](https://img.shields.io/badge/平台-Windows%20%2F%20macOS%20%2F%20Linux-lightgrey) ![版本](https://img.shields.io/badge/版本-v1.4.0-green) ![技术](https://img.shields.io/badge/技术-WebRTC%20%2B%20Node.js-orange)
...
| 👀 观看体验 | 主画面支持**全屏**、**系统级画中画（PiP）**、**画面比例**（自适应 / 16:9 / 原始 / 拉伸），每格独立音量 |
| 🎙️ 内置语音 | 麦克风通话 + 说话指示灯 + 独立音量，**AEC 回声消除**，共享时不会听到自己的回音 |
| 🎞️ 影院式 UI | 深色大屏舞台 + 左侧成员列表 + 底部控制栏，专注观看 |
| 🔗 房间系统 | **单服务器单房间**：取消房间号，主机自选端口，好友首页直接填 IP:端口 加入 |
| 🌐 跨网络 | **内嵌 Radmin VPN 一键联机**（推荐），也可蒲公英 / 米西 组网或 SakuraFrp 内网穿透（见文档） |
| 🖧 连接方式 | 主机自选端口（默认 3000）；好友首页输入 IP:端口 直连，重启后自动进入 |
| 🔄 自动更新 | 安装版支持自动检查 + 静默升级（国内代理加速） |
| 📋 Debug 版 | 每次迭代发布带控制台 + 日志 + DevTools 的调试 exe |

> 🎙️ **内置语音**：v1.4.0 起内置麦克风通话，说话指示灯 + 独立音量；麦克风开启 AEC 回声消除、本地不回放自己的声音，
> 共享窗口声音时自动静音其他应用（含本机语音回放），因此**共享时既不会听到自己的回音，也不会把朋友的声音当共享音频传回**。
...
### 方案〇：Radmin VPN 内嵌联机（主推，零配置）

GinyScreen 桌面版**内置官方 Radmin VPN**（免费虚拟局域网），在软件里即可完成全部操作，无需去官网注册：

1. 打开「**🌐 联机**」（房间顶部或首页）
2. 未安装时点「**安装 Radmin VPN**」（仅首次，弹 UAC 点“是”）
3. 开房者点「**＋ 创建网络**」，输入网络名称和密码 → 把名称/密码发给好友
4. 好友点「**加入网络**」填入同一名称/密码
5. 双方点「**刷新状态**」看到虚拟 IP（形如 `26.x.x.x`）
6. 开房者在「服务器设置」确认「本机开服」；好友在「服务器设置」选「连接朋友的服务器」，点「**填入我的IP:3000**」自动填好开房者的地址 → 保存并重启即进入房间

> 所有成员在同一 Radmin 网络内，屏幕画面走 WebRTC P2P 直连，**不经过任何服务器**；只有开服者需要把端口（默认 3000）作为服务端口（虚拟网内无需公网穿透）。
...
| ~~ZeroTier~~ | ~~免费~~ | 已弃用：国内连接不稳定，好友端易卡在 REQUESTING CONFIGURATION，不再推荐 |
...
> 构建本地依赖：`resources/`（Radmin 安装包、gv-helper.exe、NAudio DLL）**不入 git 仓库**，
> 全新克隆后需从本机或上一版本归档拷贝 `resources/` 才能打包 exe。
```
...
| 版本 | 说明 |
|------|------|
| v1.4.0 | 内置语音（麦克风+说话指示+独立音量+AEC 防回音）；内嵌 Radmin VPN 一键组网；观看端全屏/画中画/画面比例；共享窗口只传该窗口声音（自动静音其他应用）；共享点击反馈与防重复；自动更新镜像+重试+安装校验 |
